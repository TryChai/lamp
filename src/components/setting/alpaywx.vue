  ...
  <div class="con_bg">
    ...
    <form>
      <div class="cu-form-group">
        <div class="title hongx sx">{{$h('微信账号')}}</div>
        <van-cell-group class :border="false">
          <van-field  @blur="windowScorll" 
            v-model="userInfoData.wechat"
            type="text"
            placeholder="请填写正确的微信账号(将加密保存)"
            clearable
          />
        </van-cell-group>
      </div>
      ...
      <div class="cu-form-group" style="border:none">
        <div class="title beiz">
          <div style="color: #999;">{{$h('温馨提示：请上传微信收款二维码的截图，请勿裁剪涂改，保证图片信息清晰显示，否则无法审核通过')}}</div>
        </div>
      </div>
      <div class="bg-gradual-write up_img_con">
        <div  class="box_alea">
          <div class="up_bor" @click="imagePreview(wximgList)" :style="'background-image:url(' + $fnc.getImgUrl(wximgList) +')'" v-show="wximgList!=''" ></div>
          <div class="cu-tag close_img bg-red" v-show="wximgList!=''">
            <van-icon name="cross" color="#fff" @click="closeImg('微信')" size="14"/>
          </div>
          <van-uploader :after-read="onRead">
            <div class="up_bor wx_bg" v-show="wximgList==''">
              <div class="wx_up_img"></div>
              <div class="up_text">{{$h('上传微信收款码')}}</div>
            </div>
          </van-uploader>
        </div>
        ...
        <!-- <div  class="box_alea">
          ...
          <van-uploader :after-read="onReadA">
            <div class="up_bor" v-show="alipyImgList==''">
              ...
              <div class="up_text">上传支付宝收款码</div>
            </div>
          </van-uploader>
        </div> -->
      </div>
...
export default {
  ...
  methods: {
    ...
    subInfo() {
    if (
        this.userInfoData.wechat == "" ||
        this.userInfoData.wechat.length > 25
      ) {
        this.$toast.fail("微信账号不能为空，且最多不超过25个字符");
        return false;
      } else {
        var params = {
          wechat: this.userInfoData.wechat,
          wechat_pic: this.wximgList || ""
        };

        this.$api.getSetting.setWxSetting(params).then(res=>{
            if(res.code==200){
  ...
  created() {
      ...
      this.wximgList=info.wechat_pic || '';
