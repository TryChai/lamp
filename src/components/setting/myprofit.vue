  ...
  <div class="con_bg">
    ...
    <form>
      <div class="cu-form-group margin-top">
        <div class="title">{{ $h("新密码") }}</div>
        <van-cell-group class :border="false">
          <van-field
            @blur="windowScorll"
            v-model="newpassword"
            type="password"
            :placeholder="$h('请输入新密码')"
            clearable
          />
        </van-cell-group>
      </div>
      ...
      <div
        v-if="$store.state.config.sms && $store.state.config.sms.xgmmdxyz == 1"
      >
        <div class="cu-form-group">
      ...
      <div v-else>
        <div class="cu-form-group" v-if="!is_mrmm">
  ...
  methods: {
    ...
    subInfo() {
      if (this.password == "") {
        ...
        this.$toast.fail("密码非法,请从新输入");
        return false;
      } else if (this.newpassword != this.password) {
        this.$toast.fail("两次输入的不一致,请从新输入");
        return false;
      }
      var params = {};
      params.new_password = this.$getCode(this.password);
      params.code = this.code;
      params.unicode = this.wyz;
      params.tel = this.tel;
      params.quhao = this.quhao;
      params.old_password = this.$getCode(this.oldpassword);
      this.$api.getSetting.changePassword(params).then((res) => {
        if (res.code == 200) {
    ...
    get_account_state() {
      this.$api.getUser.get_account_state({}).then((res) => {
        if (res.code == 200) {
          this.is_mrmm = res.result.is_mrmm;
        }
      });
    },
  },
