<template>
  <div class="cut" id="cut">
    <mescroll-vue ref="mescroll" :down="mescrollDown" :up="mescrollUp" @init="mescrollInit" id="cut-mescroll">
      <van-nav-bar left-text left-arrow class="navbar" title="砍价活动" @click-left="toBack"></van-nav-bar>
      <div class="cut_body" v-show="banner">
        <div class="cut_top">
          <img :src="$fnc.getImgUrl(banner)" alt />
        </div>

        <div class="cut_box">
          <div class="cut_box_item" v-for="(item,i) in cut_list" :key="i">
            <cut_item :info="item"></cut_item>
          </div>
        </div>
      </div>
    </mescroll-vue>
      ...
      return {
        ...
        mescrollUp: {
          ...
          empty: {
            ...
            warpId: "cut-mescroll", //父布局的id (1.3.5版本支持传入dom元素)
        ...
        this.$api.getShop
          ...
          .then(res => {
            if (res.code == 200) {
              ...
              // 如果是第一页需手动置空列表
              if (page.num === 1) this.cut_list = [];
              // 把请求到的数据添加到列表
              this.cut_list = this.cut_list.concat(arr);
              // 数据渲染成功后,隐藏下拉刷新的状态
              this.$nextTick(() => {
                setTimeout(() => {
