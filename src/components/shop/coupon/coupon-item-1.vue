    ...
    <div v-for="(r,i) in item.red" :key="i">
      <div class="fx couponItem_detail" v-if="r.pid > 0">
        ...
        <div class="fx ci_div_info">
          ...
          >开始时间：{{$fnc.getTimeFormat(r.begin_time)}}</p>
          <p
            class="ci_div_info_p3"
          >截止时间：{{$fnc.getTimeFormat(r.end_time)}}</p>
        </div>
        <div class="fx ci_div_btn">
          <van-button type="warning" size="mini" round @click="goToUsed(r.pid,r)">去使用</van-button>
...
export default {
  ...
  methods: {
    ...
    goToUsed(id,row) {
      if(row.product_packet>0){
        this.$store.commit('setCouponItem',row)
        this.$router.push(`/shop/shopsearch?coupon=${row.product_packet}`)
        return
      }
      if (id > 0) {
