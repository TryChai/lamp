<template>
  <div class="fx searc_conn searc_content">
      <van-nav-bar
      title="每日上新"
      left-arrow
      class="navbar"
    ...
    <mescroll-vue
      ...
      id="searc_conn"
    >
      <indexshoplist :top_shoplist="list" class="shop-search-con" v-if="list.length>0" />
    </mescroll-vue>
  </div>
</template>
...
export default {
    ...
    return {
      ...
      mescrollUp: {
        ...
        empty: {
          ...
          warpId: "searc_conn", //父布局的id (1.3.5版本支持传入dom元素)
  ...
  methods: {
    ...
    upCallback(page, mescroll) {
      ...
      this.$api.getShop.getShopNewDay(params).then(res => {
        if (res.code == 200) {
