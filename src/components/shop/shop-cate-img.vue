<template>
  <div class="shop-cate-box" id="shop-cate-box" :class="{sipplierHead:$route.query.id}">
    <div style="flex: 1;overflow: hidden;">
      <div style="height: 100%;overflow: initial;display: flex;flex-flow: column;justify-content: flex-start;">
        <supplierTop v-if="$route.path == '/shop/cateimg' && $route.query.id" id="cateImgHead"></supplierTop>
        ...
        <div class="cate-header-wrapper" v-else id="cateImgHead" style="height:50px">
          <div>
            ...
            <div class="fx_3 cate-header-wrapper-tabs">
              <van-tabs v-model="active" title-active-color="#000" title-inactive-color="#bababa" @change="changeTab" color="#fff" :border="false"
                  :ellipsis="false">
                <van-tab :name="item.id" :title="item.title.length<=2? toZ(item.title) :item.title" v-for="(item,i) in cate" :key="i"></van-tab>
              </van-tabs>
            </div>
        ...
        <van-tree-select class="cate-header-select" ref="container" :class="{leftShow:items.length==0}" :items="toItem(items)" @click-nav="checkLeft"
            :main-active-index.sync="active1">
          <template slot="content">
            <mescroll-vue ref="mescroll" :down="mescrollDown" :up="mescrollUp" @init="mescrollInit" id="shop-cate-box-s">
              <div class="slide_box">
              ...
              <van-sticky :container="container" :offset-top="topNumber" class="cate-search-content" :class="{notLeft:items.length==0}"
                  style="z-index:5">
                <div class="cate-search" ref="cateSearch">
                  <div class="fx menu_sear">
                    <div class="menu_text" @click="getPrice('综合')" :class="{zh_srarch:form.order_sn==''}">综合</div>
                    <div class="menu_text" @click="getPrice('销量')" :class="{xl_srarch:form.order_sn=='invent_sale_desc'}">
                      销量</div>
                    <div class="menu_text" @click="getPrice" :class="[form.order_sn=='price_asc' || form.order_sn=='price_desc'?'xl_srarch ':'']">
                      价格
                      <span>
    ...
    <div class="cate-footer">
      ...
      <div class="shop-cate" id="cate-footer" :class="{showPop:showCard}">
        <van-popup v-model="showCard" position="bottom" get-container="#cate-footer" id="cate-footer-box">
          <div class="card-img-pop">
            <!-- {{shopCardList}} -->
            <div class="fx card-img-pop-head">
              <p>{{cardList[0]&&cardList[0].info ?cardList[0].info.shop_title : 'xxx'}}
              </p>
              <p @click="delAll">
    ...
    <van-popup v-model="show" position="bottom" get-container="body" :overlay="true" class="p-p" style="border-radius: 5px 5px 0 0;">
      <shopDetailsSku :shopInfo="shopRow" v-if="show" @closePop="getClosePop" ref="childs" :toAut="0"></shopDetailsSku>
    </van-popup>
  </div>
</template>
...
export default {
  ...
  data () {
    return {
      ...
      cateId: "",
      slideList: [],
      topNumber: this.$route.query.id ? 0 : 50
    };
  },
  computed: {
    ...mapState({
      ...
      this.$api.getShopCate
        ...
        .then(res => {
          if (res.code == 200) {
            ...
            if (this.$route.path == '/shop/cateimg' && this.$route.query.id) {
              //如果为骑手页都显示1级
              this.items = arr;
              this.cateId = arr[0].id;
              this.piclink = arr[0].piclink;
              this.mescroll.resetUpScroll();
            } else {
            ...
            this.container = this.$refs.mescroll;
          }
        });
    },
    get_slide (id) {
