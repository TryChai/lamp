...
<template>
  <div class="vue-box">
    <img :src="imgUrl" alt="" id="img" :style="{'border-radius': imground == true?'50%':''}">
    <input @blur="windowScorll" type="file" class="file" accept="image/*" @change="change($event)">
  </div>
</template>
<script>
  ...
  methods: {
    ...
    initilize (opt) {
      ...
      this.cropper = new Cropper(this.preview, {
        ...
        viewMode: 2,
        guides: true,
        cropBoxResizable: this.cropBoxResizable, //是否通过拖动来调整剪裁框的大小
