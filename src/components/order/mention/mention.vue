<template>
  <div class="order_list_con" id="orderList">
    <mescroll-vue ref="mescroll" :down="mescrollDown" :up="mescrollUp" @init="mescrollInit" class="scol">
      <van-nav-bar title="自提管理" left-text="" left-arrow class="navbar" :border="false" @click-left="toBack">
      </van-nav-bar>
      <div>
...
<script>
  ...
  import orderItem from '@/components/order/mention/mention_item'
  export default {
    data() {
      return {
        ...
        mescrollUp: {
          ...
          empty: {
            ...
            warpId: "orderList", //父布局的id (1.3.5版本支持传入dom元素)
    ...
    methods: {
      ...
      upCallback(page, mescroll) {
        ...
        this.$api.getOrder.get_mention_list(params).then(res => {
          if (res.code == 200) {
            let arr = res.result;
            // 如果是第一页需手动置空列表
