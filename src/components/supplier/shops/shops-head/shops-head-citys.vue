  ...
  <div class="shops-head-citys">
    ...
    <div class="shops-addresss">
      <div :class="{addressActive:show}">
        <div v-for="(item,i) in provinceList[provinceIndex].z[cityIndex].z" @click="clickArea(item)" :class="{areaActive:(area==item.t)}" :key="i">
          {{item.t}}</div>
      </div>
    </div>
    ...
    <div class="shops-citys-box fx">
      <ul class="shops-citys-box-province">
        ...
        <li v-for="(item,i) in provinceList" :key="i" @click="province=item.t,provinceIndex=i,cityIndex=0,isHit=false;city=item.z[0].t"
            :class="{provinceActive:(item.t==province && !isHit)}">{{item.t}}</li>
      </ul>
      ...
      <ul class="shops-citys-box-city" v-show="!isHit">
        <li v-for="(item,i) in provinceList[provinceIndex].z" :key="i" @click="clickCity(item,i)" :class="{provinceActive:item.t==city}">
          <p>{{item.t}}</p>
