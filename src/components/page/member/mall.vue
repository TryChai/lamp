<template>
  <div style="display:flex;flex-flow:column;height: 100%;">
    <div class="container member_bj" style="max-height:100%">
      <div class="member_shop_top">
        <div class="member_shop_top_bj">
          <img :src="$fnc.getImgUrl(vip.background)" alt />
        </div>
      ...
      <div class="member_shop_body" :style="{'background-color':vip.color}">
        <div v-if="ycxtiSOpen=='0'">
          <div class="member_shop_item" v-for="(item,k) in product" @click="$router.push({path:'/shop/shopdetails',query:{id:item.id}})" :key="k">
            <div class="member_shop_item_left">
              <img :src="$fnc.getImgUrl(item.piclink)" alt />
            </div>
            <div class="member_shop_item_right">
              <p>{{item.title}}</p>
              <div v-if="item.sub_title != '' && item.sub_title != null && item.sub_title != undefined " class="member_shop_item_right_recommend">
                <p>{{item.sub_title}}</p>
              </div>
              <div class="member_shop_item_right_bottom">
                ...
                <div class="member_shop_item_right_bottom_btn">
                  <p @click="$router.push({path:'/shop/shopdetails',query:{id:item.id}})">
                    立即抢购</p>
                </div>
              </div>
            </div>
        ...
        <div class="member_shop_body-yc" v-else>
        </div>
      </div>
    </div>
  </div>
